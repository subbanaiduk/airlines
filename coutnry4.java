...
public class SameColorFlag extends Configured implements Tool {
  ...
  public static class Map extends Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    public void map(LongWritable key, Text value,
	  ...
	  int tr = Integer.parseInt(fields[28]);
	  int bl = Integer.parseInt(fields[29]);
	  if(tr == bl) {
		word.set(fields[0]);  
  ...
  public static class Reduce extends Reducer<Text, IntWritable, Text, IntWritable> {
    ...
    public void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
      ...
      context.write(new Text("Total Number of Coutries has same top-left and bottom-righ color in the flag: "), new IntWritable(sum));
    }
  }
}